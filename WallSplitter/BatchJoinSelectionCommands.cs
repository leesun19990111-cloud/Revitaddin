...
{
    // 리본 "일괄결합" 패널의 작은 버튼 두 개 - 창을 열지 않고 **지금 선택한 요소**의 양쪽 끝을 바로
    ...
    internal static class BatchJoinSelectionRunner
    {
        public static Result Run(ExternalCommandData commandData, ref string message, bool allow)
        {
            ...
            if (targets.Count == 0)
            {
                TaskDialog.Show("일괄결합",
                    unsupported > 0
                        ? "고른 요소 중에 벽·보·가새가 없습니다.\n끝 결합을 설정할 수 있는 것은 벽과 구조 프레임(보·가새)뿐입니다."
                        : "먼저 벽이나 보를 선택한 뒤 눌러주세요.\n유형 단위로 한 번에 걸려면 '일괄결합' 버튼으로 창을 여세요.");
                return Result.Succeeded;
            }

            BatchJoinService.ApplyResult result;
            using (Transaction tx = new Transaction(doc, allow ? "일괄결합 - 결합 허용" : "일괄결합 - 결합 금지"))
            {
                tx.Start();
            ...
            TaskDialog.Show("일괄결합", result.Summary(allow));
            return Result.Succeeded;
        }
    }
