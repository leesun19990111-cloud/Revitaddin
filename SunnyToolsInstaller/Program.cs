...
// 패턴 형상 연산은 모든 연도에서 Clipper2Lib을 사용한다. 누락된 패키지를 성공으로
// 표시하면 설치 후 Revit에서 명령 시작 자체가 실패하므로 복사 전에 필수 파일을 확인한다.
string[] requiredPayloadFiles = { "WallSplitter.dll", "Clipper2Lib.dll" };
...
{
    Console.WriteLine("Revit이 실행 중이라 설치를 중단합니다.");
    Console.WriteLine("열린 작업을 저장하고 Revit을 완전히 종료한 뒤 설치 프로그램을 다시 실행하세요.");
    Pause();
    return 2;
}
...
foreach (string year in targetYears)
{
    ...
    string[] missingPayloadFiles = requiredPayloadFiles
        .Where(file => !File.Exists(Path.Combine(sourceDir, file)))
        .ToArray();
    if (missingPayloadFiles.Length > 0)
    {
        Console.WriteLine($"[{year}] 설치 실패: 필수 파일이 없습니다 ({string.Join(", ", missingPayloadFiles)}).");
        failed.Add(year);
        continue;
    }
...
if (installed.Count > 0)
{
    ...
    Console.WriteLine("해당 Revit을 (재)시작하면 상단 \"Sunny Tools\" 탭에 패턴 스튜디오를 포함한 도구들이 나타납니다.");
}
else
{
